...
@Injectable()
export class SurveyCreateConstraintGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    /* 권한 검증 */
    this.utilRepository.surveyPermissionGrantsValidation(subscription, ValidateActionType.Create);
    ...
    return new Promise(async (resolve, reject) => {
      const callback = (data: { [key in PlanGrantConstraintsType]: number }) => {
        if (PlanGrantConstraintsType.PerQuestionForSurvey in data) {
          if (request.body.questions.length <= data[PlanGrantConstraintsType.PerQuestionForSurvey]) {
            resolve(true);
          } else {
      ...
      try {
        await this.utilRepository.surveyPlanGrantsValidation(
          user.id,
          [PlanGrantConstraintsType.SurveyCreate, PlanGrantConstraintsType.PerQuestionForSurvey],
          callback,
        );
      } catch (error) {
        reject(error);
...
export const SurveyCreateConstraintValidation = () => applyDecorators(UseGuards(SurveyCreateConstraintGuard));
